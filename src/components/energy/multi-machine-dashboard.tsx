...
// Ubah bagian CustomTooltip di MachineLineChart untuk menggunakan styling inline
...
export default function MultiMachineDashboard() {
  ...
  const getFormattedMetricValue = (machine: Machine, metric: string) => {
    const value =
      machineMetrics[machine.id]?.[metric as keyof MachineMetrics] ?? 0;
      ...
      <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
        ...
        <Tabs defaultValue="dashboard" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="kWh">Energy (kWh)</TabsTrigger>
            <TabsTrigger value="kW">Power (kW)</TabsTrigger>
            <TabsTrigger value="voltage">Voltage (V)</TabsTrigger>
            <TabsTrigger value="pf">Power Factor</TabsTrigger>
            <TabsTrigger value="ampere">Current (A)</TabsTrigger>
          </TabsList>
            ...
            <div className="flex flex-col space-y-4 md:flex-row md:space-x-4 md:space-y-0">
              ...
              <div className="flex-1">
                <Card>
                  ...
                  <CardContent className="pt-0">
                    {layout === "grid" ? (
                      <div className={`grid gap-4 ${getGridTemplateColumns()}`}>
                        {selectedMachines
                        ...
                        <Card key={machine.id} className="overflow-hidden">
                          <CardHeader className="p-4 pb-2">
                            <div className="flex items-center justify-between">
                              ...
                              <Badge variant="outline" className="ml-2">
                                {getFormattedMetricValue(machine, option.value)}
                                {option.value === "pf"
                                  ? ""
                                  : ` ${option.value}`}
                              </Badge>
